...
<File>
	...
	<CreationDate>2020-01-02 22:18:36Z</CreationDate>
</File>
*/
#endregion
...
namespace Calcium.InversionOfControl
{
	...
	/// for <c>Type</c> caches; enabling it to work alongside .NET Core
	...
	/// </summary>
	public class WeakReferencingContainer : IContainer
	{
		readonly ConditionalWeakTable<Type, ResolverDictionary> typeDictionary 
			= new ConditionalWeakTable<Type, ResolverDictionary>();

		readonly Dictionary<string, WeakReference<Type>> keyDictionary
		...
		readonly ConditionalWeakTable<Type, List<string>> cycleDictionary
			= new ConditionalWeakTable<Type, List<string>>();

		readonly ConditionalWeakTable<Type, ConstructorInvokeInfo> constructorDictionary
			= new ConditionalWeakTable<Type, ConstructorInvokeInfo>();
		readonly ConditionalWeakTable<Type, List<PropertyInfo>> injectablePropertyDictionary
			= new ConditionalWeakTable<Type, List<PropertyInfo>>();
		readonly ConditionalWeakTable<string, Action<object, object>> propertyActionDictionary
			= new ConditionalWeakTable<string, Action<object, object>>();
		readonly ConditionalWeakTable<Type, PropertyInfo[]> propertyDictionary
		...
		{
			/* We need a reverse lookup (another WeakReferencingTable for keys. */
		...
		public bool TryResolve<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(
			out T result, 
			string key = null)
		{
		...
		public bool TryResolve([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] Type type, 
							   out object result, 
							   string key = null)
		{
		...
		object ResolveCore([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] Type type, 
						   string key, 
						   bool raiseExceptionIfNotResolved = true)
		{
			key = GetKeyValueOrDefault(key);
		...
		{
			if (!keyDictionary.TryGetValue(key, out var weakReference))
			{
				return null;
			}

			weakReference.TryGetTarget(out Type result);
			return result;
		}
		...
		void ResolveProperties(object instance, Dictionary<string, object> resolvedObjects)
		{
			...
			try
			{
				...
				if (!hasCachedInjectableProperties)
				{
...
#endif
						propertyDictionary.SetValue(type, properties);
					}
					...
					injectablePropertyDictionary.SetValue(type, injectableProperties);
				}
			}
			finally
			{
			...
			foreach (PropertyInfo propertyInfo in injectableProperties)
			{
												...
												out Action<object, object> setter))
				{
					...
					propertyActionDictionary.SetValue(fullPropertyName, setter);
				}
		...
		object BuildUp([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] Type type, string key)
		{
			if (type == null)
			{
		...
		object Instantiate([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] Type type)
		{
